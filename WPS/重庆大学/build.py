# -*- coding: utf-8 -*-
"""重庆大学2025招生宣传PPT — WPS COM自动化构建脚本"""
import os
import json
import pythoncom
...
# ===== 配置区 =====
OUT = r"D:\A-资料\A-claudewenjian\PPT制作\重庆大学\work"
...
def run():
    ...
    def slide():
        """新建一页幻灯片，铺设模板背景"""
        s = ppt.Slides.Add(idx[0], 12)  # 12 = ppLayoutBlank
        idx[0] += 1
        try:
    ...
    # 3. 基础形状函数
    def txt(s, x, y, w, h, text, fs=28, color=0x333333, bold=False,
            align=1, font=FONT_BODY, spacing=1.3):
        """WPS 文本框"""
    ...
    def circle(s, x, y, size, color, text="", fs=24, tc=0xFFFFFF):
        ...
        if text:
            t2 = c.TextFrame.TextRange
            t2.Text = text
            t2.Font.Size = fs
            t2.Font.Color = tc
            t2.Font.Name = FONT_TITLE
            t2.Font.Bold = True
            t2.ParagraphFormat.Alignment = 2
        return c

    def rrect(s, x, y, w, h, color, text="", fs=18, tc=0xFFFFFF,
              bold=True, align=2, font=FONT_TITLE):
        """圆角矩形（带文字）"""
        ...
        r.Line.Visible = False
        if text:
            t2 = r.TextFrame.TextRange
            t2.Text = text
            t2.Font.Size = fs
            t2.Font.Color = tc
            t2.Font.Name = font
            t2.Font.Bold = bold
            t2.ParagraphFormat.Alignment = align
        return r
    ...
    def draw_text(s, e):
        ...
        return txt(s, e['x'], e['y'], e['w'], e['h'], e['text'],
                   ...
                   align=e.get('align', 1), font=e.get('font', FONT_BODY),
    ...
    def draw_table(s, e):
        ...
        dta = e['data']
        hdr = hex2bgr(e.get('header_color', '#004B8D'))
        row_h = h // rows
        col_w = w // cols
        for r in range(rows):
            for c in range(cols):
                ...
                is_hdr = (r == 0)
                bg_c = hdr if is_hdr else (
                    0xFFFFFF if r % 2 == 0 else hex2bgr('#F5F7FA'))
                fs = e.get('th_fs', 15) if is_hdr else e.get('td_fs', 14)
                tc = 0xFFFFFF if is_hdr else 0x333333
                al = 2 if c > 0 else 1
                rrect(s, cx, cy, col_w, row_h, bg_c)
                if val:
                    txt(s, cx + 4, cy + 2, col_w - 8, row_h - 4, str(val),
                        fs=fs, color=tc, bold=is_hdr, align=al,
    ...
    def draw_card_list_wide(s, e):
        ...
        cols = ['#004B8D', '#C41E3A', '#0088A0', '#2E8B57', '#D4A82A',
                '#E06020', '#004B8D', '#0088A0', '#C41E3A', '#2E8B57']
        for i, item in enumerate(items):
            y = sy + i * (ih + gap)
            col = hex2bgr(cols[i % len(cols)])
            circle(s, 100, y + 6, 32, col, item['num'], fs=13)
            txt(s, 148, y + 4, 300, 28, item['title'], fs=24,
                color=title_c, bold=True, font=FONT_TITLE)
            txt(s, 148, y + 30, 680, 16, item['sub'], fs=16,
                color=sub_c, bold=False, font=FONT_BODY)

    def draw_tagline_bar(s, e):
        """页面底部总结条"""
        col = hex2bgr(e.get('color', '#004B8D'))
        x, y, w, h = e['x'], e['y'], e['w'], e['h']
        rrect(s, x, y, w, h, col)
        txt(s, x + 10, y + 5, w - 20, h - 10, e['text'],
            fs=14, color=0xFFFFFF, bold=False, align=2, font=FONT_BODY)
    ...
    def draw_cards_2x3(s, e):
        ...
        for i, item in enumerate(items):
            ...
            rect(s, x, y, cw, 4, col)
            txt(s, x + 10, y + 16, cw - 20, 32, item['title'],
                fs=26, color=col, bold=True, align=1, font=FONT_TITLE)
            txt(s, x + 10, y + 56, cw - 20, ch - 66, item['desc'],
                fs=17, color=hex2bgr('#333333'), bold=False, align=1,
                font=FONT_BODY, spacing=1.35)

    def draw_cards_1x4_info(s, e):
        """1行4列数字信息卡（封面用）"""
        items = e['items']
        sy = e.get('start_y', 430)
        cw = 220
        gx = 18
        start_x = (960 - (4 * cw + 3 * gx)) // 2
        for i, item in enumerate(items):
            x = start_x + i * (cw + gx)
            col = hex2bgr(item['color'])
            # 顶部细线
            ...
            # 数字
            txt(s, x, sy + 14, cw, 42, item['num'],
                fs=32, color=col, bold=True, align=2, font=FONT_TITLE)
            # 标签
            txt(s, x, sy + 58, cw, 24, item['label'],
                fs=14, color=hex2bgr('#555555'), bold=False, align=2, font=FONT_BODY)
    ...
    # 7. 保存输出
    pptx_path = os.path.join(OUT, "重庆大学2025招生宣传.pptx")
    ppt.SaveAs(pptx_path)
    print(f"PPTX: {pptx_path} ({os.path.getsize(pptx_path):,} bytes)")

    pdf_path = os.path.join(OUT, "重庆大学2025招生宣传.pdf")
    try:
