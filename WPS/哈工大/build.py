# -*- coding: utf-8 -*-
"""哈工大2025招生数据PPT — WPS COM automation."""
import os, json, pythoncom, win32com.client

OUT = r"D:\A-资料\A-claudewenjian\PPT制作\哈尔滨工业大学\work"
JSON_PATH = os.path.join(OUT, "hit_data.json")
...
def run():
    ...
    def draw_card_list_wide(s, e):
        ...
        cols = ['#00508F','#0088A0','#C8102E','#006B6B','#D4A82A','#E06020','#00508F','#0088A0','#C8102E','#006B6B']
    ...
    def draw_tagline_bar(s, e):
        col = hex2bgr(e.get('color','#00508F'))
    ...
    ROUTERS = {
        'text': draw_text, 'image': draw_image,
    ...
    pptx_path = os.path.join(OUT, "哈工大2025本科招生数据全景.pptx")
    ppt.SaveAs(pptx_path)
    print(f"PPTX: {pptx_path} ({os.path.getsize(pptx_path):,} bytes)")
    pdf_path = os.path.join(OUT, "哈工大2025本科招生数据全景.pdf")
    try:
