...
def run():
    ...
    def draw_table(s, e):
        ...
        dta = e['data']
        hdr = hex_to_bgr(e.get('header_color','#006B3F'))
        row_h = h // rows
        col_w = w // cols
        ...
        for r in range(rows):
            for c in range(cols):
                ...
                is_hdr = (r==0)
                bg_c = hdr if is_hdr else (0xFFFFFF if r%2==0 else hex_to_bgr('#F5F7F5'))
                fs = th_fs if is_hdr else td_fs
                tc = 0xFFFFFF if is_hdr else 0x333333
                bold = is_hdr
                al = 2 if c>0 else 1
                rrect(s, cx, cy, col_w, row_h, bg_c)
                if val:
                    txt(s, cx+4, cy+2, col_w-8, row_h-4, str(val), fs=fs, color=tc, bold=bold, align=al, font=FONT_TITLE if is_hdr else FONT_BODY, spacing=1.0)

    def draw_card_list_wide(s, e):
        items = e['items']; sy = e.get('start_y',95); ih = e.get('item_h',52); gap = e.get('gap',3)
        title_c = hex_to_bgr(e.get('title_color','#1A1A1A'))
        sub_c = hex_to_bgr(e.get('sub_color','#555555'))
        cols = ['#006B3F','#007F6E','#E87722','#004D2E','#009B7D','#D1651A','#006B3F','#007F6E']
    ...
    def draw_cards_1x4_info(s, e):
        items = e['items']; sy = e.get('start_y',350); cw,ch=220,150; gap=8
        for i, item in enumerate(items):
            x=30+i*(cw+gap); col=hex_to_bgr('#006B3F')
    ...
    def draw_cards_1x3_big(s, e):
        items = e['items']; sy = e.get('start_y',82); cw,ch=290,240; gap=16
        for i, item in enumerate(items):
    ...
    pptx_path = os.path.join(OUT, "南方科技大学2025年本科招生录取数据全景.pptx")
    ppt.SaveAs(pptx_path)
    print(f"PPTX: {pptx_path} ({os.path.getsize(pptx_path):,} bytes)")
    pdf_path = os.path.join(OUT, "南方科技大学2025年本科招生录取数据全景.pdf")
    try:
