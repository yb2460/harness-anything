# -*- coding: utf-8 -*-
"""HUST Admissions PPT - WPS COM automation."""
...
def run():
    ...
    def draw_table(s, e):
        ...
        dta = e['data']
        hdr = hex_to_bgr(e.get('header_color',BLUE))
    ...
    def draw_card_list_wide(s, e):
        ...
        cols = ['#004098','#BE1E2D','#E87722','#007F6E','#2B6CB0','#004098','#BE1E2D']
    ...
    def draw_tagline_bar(s, e):
        col = hex_to_bgr(e.get('color',BLUE))
        x,y,w,h = e['x'],e['y'],e['w'],e['h']
        rrect(s, x, y, w, h, col)
    ...
    def draw_cards_1x4_info(s, e):
        ...
        for i, item in enumerate(items):
            x=30+i*(cw+gap); col=hex_to_bgr(BLUE)
    ...
    pptx_path = os.path.join(OUT, "华中科技大学2025年本科招生录取数据全景.pptx")
    ppt.SaveAs(pptx_path)
    print(f"PPTX: {pptx_path} ({os.path.getsize(pptx_path):,} bytes)")
    pdf_path = os.path.join(OUT, "华中科技大学2025年本科招生录取数据全景.pdf")
    try:
