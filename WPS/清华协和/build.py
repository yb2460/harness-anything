# -*- coding: utf-8 -*-
"""PUMC Admissions PPT - WPS COM automation."""
import os, json, pythoncom, win32com.client

OUT = r"D:\A-资料\A-claudewenjian\PPT制作\清华协和\work"
JSON_PATH = os.path.join(OUT, "pumc_data.json")
...
def run():
    ...
    def draw_table(s, e):
        ...
        dta = e['data']
        hdr = hex_to_bgr(e.get('header_color',DARKBLUE))
    ...
    def draw_card_list_wide(s, e):
        ...
        cols = ['#003D5C','#C62828','#E65100','#00695C','#1A6B8A','#B8860B','#003D5C','#C62828','#E65100','#00695C']
        for i, item in enumerate(items):
            y = sy + i*(ih+gap); col = hex_to_bgr(cols[i%len(cols)])
            circle(s, 100, y+6, 32, col, item['num'], fs=13)
            txt(s, 148, y+4, 300, 28, item['title'], fs=24, color=title_c, bold=True, font=FONT_TITLE)
            txt(s, 148, y+30, 680, 16, item['sub'], fs=16, color=sub_c, bold=False, font=FONT_BODY)

    def draw_tagline_bar(s, e):
        col = hex_to_bgr(e.get('color',DARKBLUE))
        x,y,w,h = e['x'],e['y'],e['w'],e['h']
        rrect(s, x, y, w, h, col)
    ...
    def draw_cards_1x4_info(s, e):
        ...
        for i, item in enumerate(items):
            x=30+i*(cw+gap); col=hex_to_bgr(DARKBLUE)
            rect(s, x, sy, cw, 4, col)
            txt(s, x, sy+20, cw, 65, item['num'], fs=40, color=col, bold=True, align=2, font=FONT_TITLE)
    ...
    pptx_path = os.path.join(OUT, "北京协和医学院2025年本科招生录取数据全景.pptx")
    ppt.SaveAs(pptx_path)
    print(f"PPTX: {pptx_path} ({os.path.getsize(pptx_path):,} bytes)")
    pdf_path = os.path.join(OUT, "北京协和医学院2025年本科招生录取数据全景.pdf")
    try:
