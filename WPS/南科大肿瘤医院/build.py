# -*- coding: utf-8 -*-
"""南科大+深圳肿瘤医院硕博招生PPT — WPS COM automation."""
import os, json, pythoncom, win32com.client

OUT = r"D:\A-资料\A-claudewenjian\PPT制作\南科大肿瘤医院\work"
...
def run():
    ...
    def draw_table(s, e):
        ...
        dta = e['data']
        hdr = hex2bgr(e.get('header_color','#005A8B'))
    ...
    def draw_card_list_wide(s, e):
        ...
        cols = ['#005A8B','#E56717','#C41230','#007A6E','#C8922A','#5C2D82','#005A8B','#E56717','#C41230','#007A6E']
    ...
    def draw_tagline_bar(s, e):
        col = hex2bgr(e.get('color','#005A8B'))
        x,y,w,h = e['x'],e['y'],e['w'],e['h']
        rrect(s, x, y, w, h, col)
    ...
    pptx_path = os.path.join(OUT, "南科大肿瘤医院硕博招生2020-2026.pptx")
    ppt.SaveAs(pptx_path)
    print(f"PPTX: {pptx_path} ({os.path.getsize(pptx_path):,} bytes)")
    pdf_path = os.path.join(OUT, "南科大肿瘤医院硕博招生2020-2026.pdf")
    try:
