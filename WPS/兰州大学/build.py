# -*- coding: utf-8 -*-
"""Lanzhou University PPT - WPS COM automation."""
import os, json, pythoncom, win32com.client

OUT = r"D:\A-资料\A-claudewenjian\PPT制作\兰州大学\work"
JSON_PATH = os.path.join(OUT, "lzu_data.json")
...
def run():
    ...
    def draw_table(s, e):
        ...
        dta = e['data']
        hdr = hex2bgr(e.get('header_color',DARKBLUE))
    ...
    def draw_cards_2x3(s, e):
        ...
        for i, item in enumerate(items):
            ...
            txt(s, x+10, y+52, cw-20, ch-62, item['desc'], fs=18, color=hex2bgr('#333333'), bold=False, align=1, font=FONT_BODY, spacing=1.4)
    ...
    pptx_path = os.path.join(OUT, "兰州大学扎根西部的百年坚守.pptx")
    ppt.SaveAs(pptx_path)
    print(f"PPTX: {pptx_path} ({os.path.getsize(pptx_path):,} bytes)")
    pdf_path = os.path.join(OUT, "兰州大学扎根西部的百年坚守.pdf")
    try:
